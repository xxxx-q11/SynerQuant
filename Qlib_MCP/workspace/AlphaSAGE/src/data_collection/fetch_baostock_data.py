...
class DataManager:
    ...
    def _fix_constituents(self) -> None:
        ...
        # Copy constituent files from base data if they exist
        if self._qlib_path is not None:
            base_instruments_path = f"{self._qlib_path}/instruments"
            if os.path.exists(base_instruments_path):
                for p in Path(base_instruments_path).iterdir():
                    if p.stem == "all":
                        continue
                    # Copy the file to the export directory
                    target_path = Path(path) / p.name
                    shutil.copy2(p, target_path)
                    print(f"Copied {p.name} to export directory")

        # Update all constituent files (including newly copied ones)
        for p in Path(path).iterdir():
            ...
            print(f"Updated {p.name} with latest date: {today}")
