...
class QLibStockDataCalculator(AlphaCalculator):
    ...
    def _calc_raw_IC(self, value1: Tensor, value2: Tensor) -> Tensor:
        return batch_pearsonr(value1, value2)
    
    def _calc_ICIR(self, value1: Tensor, value2: Tensor) -> float:
        aaa = batch_pearsonr(value1, value2)
        IC = aaa.mean().item()
        _std = aaa.std().item()
        return IC/_std
    ...
    def _calc_raw_rIC(self, value1: Tensor, value2: Tensor) -> Tensor:
    ...
    def calc_single_ICIR_ret(self, expr: Expression) -> float:
        value = self._calc_alpha(expr)
        return self._calc_ICIR(value, self.target_value)
    
    def calc_single_rICIR_ret(self, expr: Expression) -> float:
        value = self._calc_alpha(expr)
        return self._calc_rICIR(value, self.target_value)
        
    def calc_single_raw_IC_ret(self, expr: Expression) -> float:
        value = self._calc_alpha(expr)
        return self._calc_raw_IC(value, self.target_value)
    
    def calc_single_raw_rIC_ret(self, expr: Expression) -> float:
        value = self._calc_alpha(expr)
        return self._calc_raw_rIC(value, self.target_value)
    ...
    def calc_pool_ICIR_ret(self, exprs: List[Expression], weights: List[float]) -> float:
        with torch.no_grad():
            ensemble_value = self.make_ensemble_alpha(exprs, weights)
            return self._calc_ICIR(ensemble_value, self.target_value)
    ...
    def calc_pool_rICIR_ret(self, exprs: List[Expression], weights: List[float]) -> float:
        with torch.no_grad():
            ensemble_value = self.make_ensemble_alpha(exprs, weights)
            return self._calc_rICIR(ensemble_value, self.target_value)

    def calc_pool_raw_IC_ret(self, exprs: List[Expression], weights: List[float]) -> Tensor:
        with torch.no_grad():
            ensemble_value = self.make_ensemble_alpha(exprs, weights)
            return self._calc_raw_IC(ensemble_value, self.target_value)
        
    def calc_pool_raw_rIC_ret(self, exprs: List[Expression], weights: List[float]) -> Tensor:
        with torch.no_grad():
            ensemble_value = self.make_ensemble_alpha(exprs, weights)
            return self._calc_raw_rIC(ensemble_value, self.target_value)
    ...
    def calc_pool_all_ret_raw(self, exprs: List[Expression], weights: List[float]) -> Tuple[Tensor, Tensor]:
        with torch.no_grad():
            ensemble_value = self.make_ensemble_alpha(exprs, weights)
            return self._calc_raw_IC(ensemble_value, self.target_value), self._calc_raw_rIC(ensemble_value, self.target_value)