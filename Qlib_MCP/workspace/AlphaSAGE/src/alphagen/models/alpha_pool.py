...
class AlphaPool(AlphaPoolBase):
    ...
    def to_dict(self) -> dict:
        expr_strs = [str(expr) for expr in self.exprs[:self.size]]
        ics = list(self.single_ics[:self.size])
        # 方便下游使用：直接提供 {表达式字符串: ic} 的映射
        expr_ic_map = {expr: ic for expr, ic in zip(expr_strs, ics)}

        return {
            # 表达式列表（与之前兼容）
            "exprs": expr_strs,
            # 新增：表达式到 IC 的映射，便于直接按表达式查询
            "expr_ic": expr_ic_map,
            "weights": list(self.weights[:self.size]),
